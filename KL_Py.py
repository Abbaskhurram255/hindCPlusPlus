...
tab = TAB = "\t"
two_dim = list|tuple|set
# to allow readable list-in-list, tuple-in-tuple types: list[two_dim], tuple[two_dim]
...
def run_process(command: str|list[str], new_window: bool = False, **kwargs) -> None:
	...
	kwargs["shell"] = any(kwargs.pop(k, False) for k in ['cmd', 'internal', 'shell'])
	if kwargs["shell"] and isinstance(command, list):
	...
	try:
		...
		#print(f"Failed to perform command `{' '.join(command) if isinstance(command, list) else command}`")
