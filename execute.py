...
def execute(filename: str) -> None:
	keys: dict[str, str] = {
	   ...
	   "__f(?:mt)?__": "__format__",
		...
		# sequence!
		r"(?<=(?<!\bi|(?<=ag)a)\S )(?:nahi[ _]?(?:hen? )?(?:hen?|ba?ra?ba?r(?: hen?)?)?|(?:is|ai)n'?t)(?= +\S(?!(?:ndar|(?:ith)?n|nder|armya{1,2}n|eech|issa)\b))": "!=",
		# if it works, don't touch it
		# sequence
		# equality_keyword=he|brbr|barabar
		r"(?<=(?<!\bi|(?<=ag)a|(?<=war)n|\b(?<=na)h|(?<=pak)k|(?<=(?<=[wa])ak)a)\S )(?:(?:hen? )?(?:ba?ra?ba?r|hen?)(?: hen?)?)(?= +\S)": "==",
		...
		r"(?<=\S )\b(?:pakka|wa{1,2}kai) (?:(?:hen? )?(?:ba?ra?ba?r|hen?)(?: hen?)?)\b(?= +\S)": "is",
		# sequence
		r"aur(?= +\S)": "and",
		r"ya(?= +\S)": "or",
	...
	with open_case_ins(filename, "r") as file:
		...
		for i, string in old_enumerate(strings):
			...
			if re.search(r"\{[^\}]+\}", strings[i]):
					...
					for templt in templates_found_in_string:
						...
						# sum (is|he): {sum}
						processed_templt: str = replace(replace(templt, r"\{(?<placeholder_slash_varname>[^\{\}]+)(?<! ) +(?<separator>is|hen?) *:? *\}", "$placeholder_slash_varname $separator: {$placeholder_slash_varname}####"), "####", "")
						...
						# ^ a bug fix
						strings[i] = replace(strings[i], re.escape(templt), processed_templt) # escaping here is mandatory
						...
						# reversing
						strings[i] = replace(strings[i], r"\[\.?(?:k[aeio]\:?[_ ])?(?:ula?t[ai]?|reversed?)\]", "[::-1]")
						...
						strings[i] = replace(strings[i], r"\[\.?(?:(?:shuru?(?:wat)?(?:[_ ]?(?:(?:k|wal)a|me))?|n)\:)?(?<n>\d+)(?:st|nd|rd|h?la|th|th?a|s?ra|wa)\]", "[$n-1]")
						strings[i] = replace(strings[i], r"\[\.?(?:(?:a{1,2}kh(?:ir|ri)(?:[_ ]?(?:(?:k|wal)a|[sm]e))?|\-n)\:)(?<n>\d+)(?:st|nd|rd|h?la|th|th?a|s?ra|wa)\]", "[-$n]")
		...
		code = replace(code, r"(?<A>(?:[\w\-\.]+|[\[\"\'](?:[\"\'\w\-\., ][\]]*)+[\]\"\'])) (?:me|ke_(?:beech|da?rmya{0,2}n))(?:(?: hen?)?(?: (?:mojud|shamil)(?: hen?)?|hen?)?)? nahi(?:(?: hen?)?(?: (?:mojud|shamil)(?: hen?)?|hen?)?)? (?<B>(?:[\[\"\'](?:[\"\'\w\-\.,][\]]*)+[\]\"\']|[\w\-\.]+))(?:(?: hen?)?(?: (?:mojud|shamil)(?: hen?)?|hen?)?)?\b", "not($B in $A)")
		# bonus negation (negation v2 \|/ )
		# \/ A-me-B-nahi
		code = replace(code, r"(?<A>(?:[\w\-\.]+|[\[\"\'](?:[\"\'\w\-\., ][\]]*)+[\]\"\'])) (?:me|ke_(?:beech|da?rmya{0,2}n))(?:(?: hen?)?(?: (?:mojud|shamil)(?: hen?)?|hen?)?)? (?<B>(?:[\[\"\'](?:[\"\'\w\-\.,][\]]*)+[\]\"\']|[\w\-\.]+))(?:(?: hen?)?(?: (?:mojud|shamil)(?: hen?)?|hen?)?)? nahi\b", "not($B in $A)")
		...
		code = replace(code, r"(?<B>(?:[\[\"\'](?:[\"\'\w\-\.,][\]]*)+[\]\"\']|[\w\-\.]+))(?:(?: hen?)?(?: (?:mojud|shamil)(?: hen?)?|hen?)?)? nahi(?:(?: hen?)?(?: (?:mojud|shamil)(?: hen?)?|hen?)?)? (?<A>(?:[\w\-\.]+|[\[\"\'](?:[\"\'\w\-\., ][\]]*)+[\]\"\'])) (?:me|ke_(?:beech|da?rmya{0,2}n))(?:(?: hen?)?(?: (?:mojud|shamil)(?: hen?)?|hen?)?)?\b", "not($B in $A)")
		# \^^^/ bonus negation (negation v1): B-nahi-A-me
		# as compared to
		# \V/ B-A-me-nahi (negative v2 down below \|/ )
		code = replace(code, r"(?<B>(?:[\[\"\'](?:[\"\'\w\-\.,][\]]*)+[\]\"\']|[\w\-\.]+))(?:(?: hen?)?(?: (?:mojud|shamil)(?: hen?)?|hen?)?)? (?<A>(?:[\w\-\.]+|[\[\"\'](?:[\"\'\w\-\., ][\]]*)+[\]\"\'])) (?:me|ke_(?:beech|da?rmya{0,2}n))(?:(?: hen?)?(?: (?:mojud|shamil)(?: hen?)?|hen?)?)? nahi(?:(?: hen?)?(?: (?:mojud|shamil)(?: hen?)?|hen?)?)?\b", "not($B in $A)")
		# and then the regular
		...
		code = replace(code, r" \b(?:chal(?:t[aei])? rah[aei](?: he)?)\b", ".chal_rahi_he")
		...
		# reversing
		code = replace(code, r"\[\.?(?:k[aeio]\:?[_ ])?(?:ula?t[ai]?|reversed?)\]", "[::-1]")
		# number-based mid slicing (rather than index-based)
		# comes before
		...
		code = replace(code, r"\[\.?(?:(?:shuru?(?:wat)?(?:[_ ]?(?:(?:k|wal)a|me))?|n)\:)?(?<n>\d+)(?:st|nd|rd|h?la|th|th?a|s?ra|wa)\]", "[$n-1]")
		code = replace(code, r"\[\.?(?:(?:a{1,2}kh(?:ir|ri)(?:[_ ]?(?:(?:k|wal)a|[sm]e))?|\-n)\:)(?<n>\d+)(?:st|nd|rd|h?la|th|th?a|s?ra|wa)\]", "[-$n]")
		...
		try:
			with open(TRANSLATION_FILE_DIR, "w") as file:
				translation: str = f"from KL_Py import *\n\n{code}"
				# ^ much needed
...
MODULES_DIR: str = path_to(r"venv\Lib\site-packages")
__SCRIPT_DIR__: str = path_to("execute.py")
PIP_PATH: str = path_to(r"venv\Scripts\pip")
...
class Klang(cmd.Cmd):
			...
			if re.search(r"^\-{1,2}v(?:ersion)?$", line):
				...
				if re.search(r"^[\"']?[\.\w]+[\"']?$", line):
					...
					try:
						...
						...
					run_process(f"python {__SCRIPT_DIR__} {file_to_be_compiled}")
				else:
					# otherwise,
					# interpret the line
					...
					try:
						with open(TEMP_FILE_DIR, "w") as temp_program_file:
							 ...
							 run_process(f"python {__SCRIPT_DIR__} {temp_program_file.name}", True)
					except Exception as e:
						...
					Klang().cmdloop()
		else:
	...
	def do_install(self, line) -> None:
		...
		.: Usage: i/install/add <package_name_1> [package_name_2] ...
		"""
		if not line:
			print("Usage: i/install/add <package_name_1> [package_name_2] ...")
			return
	...
	do_i = do_add = do_install
	def do_uninstall(self, line) -> None:
		"""Klang me mojud packages ko uninstall karta he
		\nUsage: u/uninstall/remove <package_name_1> [package_name_2] ...
		"""
		if not line:
			print("Usage: u/uninstall/remove <package_name_1> [package_name_2] ...")
			return
		packages: list[str] = [pkg for pkg in re.split(r"[, ]+(?:aur )?", line)]
		if os.name != "nt":
	...
	do_u = do_remove = do_uninstall
	def do_edit(self, line) -> None:
		"""Programs ko edit karne ke lie ek default editor kholta he"""
		...
		if filename and not filename.endswith(".klang"):
			filename = f"{filename}.klang"
		edit_command: str = text_editor
		# `notepad` opens notepad with the most recent file
		if filename:
			# `notepad filename`
		...
		if not filename and File(RECENTS_LOG_FILE_DIR).exists_file():
			try:
				with open(RECENTS_LOG_FILE_DIR, "r") as file:
					...
					filename = to_path(f"{most_recent_file}.klang")
					edit_command += f" {filename}"
			except:
				...
		run_process(edit_command, True)
	do_notepad = do_edit
	def do_view(self, _) -> None:
		"""
		:: Klang programs, in the end, Python me compile hote hen.
		.: Ye command bilkul pichli compiled Python file ko
		.: view karegi (agar mojud hui). Take ap ko debugging
		.: me asaani ho.
		"""
		if not File(TRANSLATION_FILE_DIR).exists_file():
			return
		text_editor: str = "nano"
		if os.name == "nt":
			text_editor = "notepad"
		translation_file = re.split(r"[\/\\]", TRANSLATION_FILE_DIR)[-1]
		command: str = f"{text_editor} {translation_file}"
		run_process(command)
	def do_kexe(self, _) -> None:
		"""
		:: Sirf Windows users ke lie: Programs ko EXE file me badalta he.
		.: Is command ko run karte hi ek program launch hoga, jispe ap is hi
		.: folder ke andar mojud ".__translated__.py" naami file ko Script path
		.: (yani pehle blank/khaal) ke andar attach karenge ("Browse" button ki madad
		.: se), or Advanced settings me jakar generate hone wale software/executable
		.: file ka naam likhege, or seedha "Convert Py to EXE" naami button --- joke
		.: program ke bilkul akhir me he --- ko click karege to apki executable file
		.: is folder ke andar ek "output" naami folder me bana di jaegi.
		"""
		if os.name != "nt":
			print("Ye command sirf Windows users ke lie hi available he. Istamal janne ke lie likhie: madad exe")
			return
		run_process("kexe.exe")
	do_build = do_produce = do_export = do_exe = do_toexe = do_banao = do_kexe
	...
	def do_clear(self, line) -> bool:
		...
		if os.name == "nt":
			...
			run_process(f"{PIP_PATH} cache purge")
		else:
