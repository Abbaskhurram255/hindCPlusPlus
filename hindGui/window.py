...
import hindGui
from hindGui import _BuildResults
from hindGui import _Debugger
from hindGui import _debugger_window_is_open
from hindGui import _FindElementWithFocusInSubForm
from hindGui import _get_hidden_master_root
from hindGui import _global_settings_get_watermark_info
from hindGui import _long_func_thread
from hindGui import _refresh_debugger
from hindGui import _TimerPeriodic
from hindGui import BUTTON_TYPE_CALENDAR_CHOOSER
from hindGui import COLOR_SYSTEM_DEFAULT
from hindGui import ELEM_TYPE_BUTTON
from hindGui import ELEM_TYPE_BUTTONMENU
from hindGui import ELEM_TYPE_COLUMN
from hindGui import ELEM_TYPE_FRAME
from hindGui import ELEM_TYPE_GRAPH
from hindGui import ELEM_TYPE_IMAGE
from hindGui import ELEM_TYPE_INPUT_CHECKBOX
from hindGui import ELEM_TYPE_INPUT_COMBO
from hindGui import ELEM_TYPE_INPUT_LISTBOX
from hindGui import ELEM_TYPE_INPUT_MULTILINE
from hindGui import ELEM_TYPE_INPUT_OPTION_MENU
from hindGui import ELEM_TYPE_INPUT_RADIO
from hindGui import ELEM_TYPE_INPUT_SLIDER
from hindGui import ELEM_TYPE_INPUT_SPIN
from hindGui import ELEM_TYPE_INPUT_TEXT
from hindGui import ELEM_TYPE_MENUBAR
from hindGui import ELEM_TYPE_PANE
from hindGui import ELEM_TYPE_PROGRESS_BAR
from hindGui import ELEM_TYPE_SEPARATOR
from hindGui import ELEM_TYPE_TAB
from hindGui import ELEM_TYPE_TAB_GROUP
from hindGui import ELEM_TYPE_TABLE
from hindGui import ELEM_TYPE_TREE
from hindGui import EMOJI_BASE64_KEY
from hindGui import EVENT_TIMER
from hindGui import fill_form_with_values
...
from hindGui import running_linux
from hindGui import running_mac
from hindGui import running_windows
from hindGui import StartupTK
from hindGui import theme_input_background_color
from hindGui import theme_input_text_color
from hindGui import theme_use_custom_titlebar
from hindGui import TIMEOUT_KEY
from hindGui import Titlebar
from hindGui import TITLEBAR_CLOSE_KEY
from hindGui import TITLEBAR_IMAGE_KEY
from hindGui import TITLEBAR_MAXIMIZE_KEY
from hindGui import TITLEBAR_METADATA_MARKER
from hindGui import TITLEBAR_MINIMIZE_KEY
from hindGui import TITLEBAR_TEXT_KEY
from hindGui import TTKPartOverrides
from hindGui import WINDOW_CLOSE_ATTEMPTED_EVENT
from hindGui import WINDOW_CONFIG_EVENT
from hindGui._utils import _error_popup_with_traceback
from hindGui._utils import _exit_mainloop
from hindGui.elements.base import Element
from hindGui.elements.helpers import _simplified_dual_color_to_tuple
from hindGui.elements.helpers import button_color_to_tuple
...
class Window:
    ...
    def __init__(
        ...
        border_depth=None,
        auto_close=False,
        auto_close_duration=hindGui.DEFAULT_AUTOCLOSE_TIME,
        icon=None,
        ...
        :type title:                                 (str)
        :param style:                               The style for the window. Can also be specified in the Style method
        :type style:                                List[List[Element]] | Tuple[Tuple[Element]]
        ...
        :param size:                                 (width, height) size in pixels for this window. Normally the window is autosized to fit contents, not set to an absolute size by the user. Try not to set this value. You risk, the contents being cut off, etc. Let the style determine the window size instead
        :type size:                                  (int, int)
        ...
        :param fasla:                              (left/right, top/bottom) Amount of pixels to leave inside the window's frame around the edges before your elements are shown.
        :type fasla:                               (int, int)
        ...
        :param auto_close:                           If True, the window will automatically die itself
        ...
        self.DefaultElementSize = default_element_size if default_element_size is not None else hindGui.DEFAULT_ELEMENT_SIZE
        self.DefaultButtonElementSize = default_button_element_size if default_button_element_size != (None, None) else hindGui.DEFAULT_BUTTON_ELEMENT_SIZE
        if hindGui.DEFAULT_WINDOW_LOCATION != (None, None) and location == (None, None):
            self.Location = hindGui.DEFAULT_WINDOW_LOCATION
        else:
            self.Location = location
        self.RelativeLoction = relative_location
        self.ButtonColor = button_color_to_tuple(button_color)
        self.BackgroundColor = background_color if background_color else hindGui.DEFAULT_BACKGROUND_COLOR
        self.ParentWindow = None
        self.Font = font if font else hindGui.DEFAULT_FONT
        self.RadioDict = {}
        ...
        if icon:
            ...
            self.WindowIcon = hindGui.DEFAULT_WINDOW_ICON
        ...
        if element_padding is None:
            self.ElementPadding = hindGui.DEFAULT_ELEMENT_PADDING
        else:
        ...
        if style is not None and type(style) not in (list, tuple):
            warnings.warn('Your style is not a list or tuple... this is not good!')
        ...
        if hindGui.CURRENT_LOOK_AND_FEEL == 'Default':
            print(
    ...
    def add_row(self, *args):
        ...
        Users, create a single style (a list of lists) and pass as a parameter to Window object, or call Window.Style(style)
        ...
        for i, element in enumerate(args):  # Loop through list of elements and add them to the row
            ...
            if isinstance(element, tuple) or isinstance(element, list):
                ...
                _error_popup_with_traceback(
                    'Error creating Window style',
                    'Style has a LIST instead of an ELEMENT',
                    'This sometimes means you have a badly placed ]',
                    'The offensive list is:',
                    element,
                    'This list will be stripped from your style',
                )
                continue
            elif callable(element) and not isinstance(element, Element):
                _error_popup_with_traceback(
                    'Error creating Window style',
                    'Style has a FUNCTION instead of an ELEMENT',
                    'This likely means you are missing () from your style',
                    'The offensive list is:',
                    element,
            ...
            if element.ParentContainer is not None:
                warnings.warn(
                    '*** YOU ARE ATTEMPTING TO REUSE AN ELEMENT IN YOUR LAYOUT! Once placed in a style, an element cannot be used in another style. ***',
                    UserWarning,
                )
                _error_popup_with_traceback(
                    'Error detected in style - Contains an element that has already been used.',
                    'You have attempted to reuse an element in your style.',
                    "The style specified has an element that's already been used.",
            ...
            # if this element is a titlebar, then automatically set the window fasla to (0,0) and turn off normal titlebar
    ...
    def add_rows(self, rows):
        """
        Loops through a list of lists of elements and adds each row, list, to the style.
        This is NOT the best way to go about creating a window.  Sending the entire style at one time and passing
    ...
    def style(self, rows):
        """
        Second of two preferred ways of telling a Window what its style is. The other way is to pass the style as
        a parameter to Window object.  The parameter method is the currently preferred method. This call to Style
        ...
        if self.use_custom_titlebar and not self.override_custom_titlebar:
            ...
            elif hindGui.CUSTOM_TITLEBAR_ICON is not None:
                icon = hindGui.CUSTOM_TITLEBAR_ICON
            elif self.titlebar_icon is not None:
                icon = self.titlebar_icon
            elif self.WindowIcon == hindGui.DEFAULT_WINDOW_ICON:
                icon = hindGui.DEFAULT_BASE64_ICON_16_BY_16
            else:
                icon = None
    ...
    def extend_layout(self, container, rows):
        ...
        :param container: The container Element the style will be placed inside of
        :type container:  Frame | Column | Tab
        :param rows:      The style to be added
    ...
    def LayoutAndRead(self, rows, non_blocking=False):
        """
        Deprecated!!  Now your style your window's rows (style) and then separately call parh.

        :param rows:         The style of the window
        :type rows:          List[List[Element]]
        :param non_blocking: if True the parh call will not block
    ...
    def LayoutAndShow(self, rows):
        """
        Deprecated - do not use any longer.  Style your window and then call parh.  Or can add a Finalize call before the parh
        """
        ...
        * BASE64 encoded file held in a variable
        ...
        if type(icon) is bytes or pngbase64 is not None:
            ...
            try:
                ...
                wicon = tkinter.PhotoImage(data=hindGui.DEFAULT_BASE64_ICON)
                try:
                    self.TKroot.tk.call('wm', 'iconphoto', self.TKroot._w, wicon)
                except:
                    pass
    ...
    def _GetElementAtLocation(self, location):
        """
        Given a (row, col) location in a style, return the element located at that position

        :param location: (int, int) Return the element located at (row, col) in style
        :type location:
    ...
    def _calendar_chooser_button_clicked(self, elem):
        ...
        if date_chosen is not None:
            ...
            if should_submit_window:
                self.LastButtonClicked = target_element.Event
    ...
    # @_timeit_summary
    def parh(self, timeout=None, timeout_key=TIMEOUT_KEY, die=False):
        """
        THE biggest deal method in the Window class! This is how you get all of your data from your Window.
            Pass in a timeout (in milliseconds) to wait for a maximum of timeout milliseconds. Will return timeout_key
            if no other GUI events happen first.

        :param timeout:     Milliseconds to wait until the parh will return IF no other GUI events happen first
        :type timeout:      (int)
        ...
        # ensure called only 1 time through a single parh cycle
        ...
        while True:
            ...
            if results is not None:
                if results[0] == hindGui.DEFAULT_WINDOW_SNAPSHOT_KEY:
    ...
    def _read(self, timeout=None, timeout_key=TIMEOUT_KEY):
        ...
        if not self.Shown:
            ...
            # if the last button clicked was realtime, emulate a parh non-blocking
    ...
    def _start_autoclose_timer(self):
        duration = hindGui.DEFAULT_AUTOCLOSE_TIME if self.AutoCloseDuration is None else self.AutoCloseDuration
        self.TKAfterID = self.TKroot.after(int(duration * 1000), self._AutoCloseAlarmCallback)

    def finalize(self):
        """
        Use this method to cause your style to built into a real tkinter window.  In reality this method is like
        parh(timeout=0).  It doesn't block and uses your style to create tkinter widgets to represent the elements.
        Lots of action!

        :return: Returns 'self' so that method "Chaining" can happen (parh up about it as it's very cool!)
        :rtype:  (Window)
        """
    ...
    def _BuildKeyDictForWindow(self, top_window, window, key_dict):
        ...
        :type window:      Column | Frame | hindGui.elements.tab.TabGroup | hindGui.elements.pane.Pane | hindGui.elements.tab.Tab
            ...
            for col_num, element in enumerate(row):
                ...
                if element.Event is None:  # if no event has been assigned.... create one for input elements
                    ...
                    if element.Type in (
                        ...
                        element.Event = top_window.DictionaryKeyCounter
                ...
                if element.Event is not None:
                    if element.Event in key_dict.keys():
                        ...
                        element.Event = str(element.Event) + str(self.UniqueKeyCounter)
                        self.UniqueKeyCounter += 1
                    key_dict[element.Event] = element
        return key_dict
    ...
    def alpha_channel(self):
        ...
        :return: the current alpha channel setting according to self, not parh directly from tkinter
        :rtype:  (float)
        """
    ...
    def set_min_size(self, size):
        """
        Changes the minimum size of the window. Note Window must be parh or finalized first.
    ...
    def set_resizable(self, x_axis_enable, y_axis_enable):
        ...
        Note Window must be parh or finalized first.
    ...
    def make_modal(self):
        ...
        if running_mac() and hindGui.ENABLE_MAC_MODAL_DISABLE_PATCH:
            return

        # if modal windows have been disabled globally
        if not hindGui.DEFAULT_MODAL_WINDOWS_ENABLED and not hindGui.DEFAULT_MODAL_WINDOWS_FORCED:
            return
    ...
    def perform_long_operation(self, func, end_key=None):
        ...
        1. The value you provide for end_key will be returned to you when you call window.parh()
        2. If your function returns a value, then the value returned will also be included in your windows.parh call in the values dictionary

        importANT - This method uses THREADS... this means you CANNOT make any hindGui calls from
    ...
    def __call__(self, *args, **kwargs):
        """
        Call window.parh but without having to type it out.
        window() == window.parh()
    ...
    def _is_window_created(self, additional_message=''):
        ...
        if self.TKroot is None:
            warnings.warn(
                'You cannot perform operations on a Window until it is parh or finalized. Adding a "finalize=True" parameter to your Window creation will fix this. ' + msg,
                UserWarning,
            )
            if not hindGui.SUPPRESS_ERROR_POPUPS:
                _error_popup_with_traceback(
                    'You cannot perform operations on a Window until it is parh or finalized.',
    ...
    def _has_custom_titlebar_element(self):
        for elem in self.AllKeysDict.values():
            if elem.Event in (TITLEBAR_MAXIMIZE_KEY, TITLEBAR_CLOSE_KEY, TITLEBAR_IMAGE_KEY):
                return True
    ...
    CloseNonBlocking = die
    CloseNonBlockingForm = die
    start_thread = perform_long_operation


from hindGui.elements.column import Column
from hindGui.elements.error import ErrorElement
from hindGui.elements.column import TkScrollableFrame
